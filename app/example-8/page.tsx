  ...
  return (
    <main className="min-h-screen bg-[#F4EFE6] text-stone-900 font-[Fraunces,Georgia,serif]">
      ...
      <header className="border-b-2 border-stone-900">
        ...
        <nav className="sticky top-0 z-50 border-b-2 border-stone-900 bg-[#F4EFE6]">
      ...
      <section id="o-nas" className="border-y-2 border-stone-900 bg-[#EAE3D2]">
        <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 sm:py-20 md:px-10">
          ...
          <RevealH2 className="mt-4 max-w-4xl font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-5xl sm:leading-[0.95] md:text-7xl">{c.about.headline}</RevealH2>
          <div className="mt-10 grid gap-10 sm:mt-12 sm:gap-12 md:grid-cols-3">
            <Reveal delay={0.15} className="space-y-5 font-[Inter] text-base leading-relaxed text-stone-700 md:col-span-2 md:columns-2 md:gap-12 [&_p]:break-inside-avoid">
              <p>{c.about.lead}</p>
            ...
            <div className="border-t-2 border-stone-900 pt-6">
              ...
              <Stagger className="mt-6 space-y-5 sm:space-y-6">
                {c.about.stats.map((s) => (
                  <StaggerItem key={s.label}>
                    <div className="flex items-baseline gap-4 border-b border-stone-400 pb-4">
                      <p className="font-[DM_Serif_Display] text-4xl leading-none sm:text-5xl">{s.value}</p>
                      <p className="font-[Inter] text-xs uppercase tracking-[0.18em] text-stone-700">{s.label}</p>
                    </div>
                  </StaggerItem>
                ))}
              </Stagger>
      ...
      <section id="nagrody" className="mx-auto max-w-screen-xl border-b-2 border-stone-900 px-4 py-16 sm:px-6 sm:py-20 md:px-10">
        ...
        <RevealH2 className="mt-4 max-w-3xl font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-5xl sm:leading-[0.95] md:text-7xl">{c.awards.headline}</RevealH2>
        <Reveal delay={0.15}>
          <p className="mt-6 max-w-2xl font-[Inter] text-base text-stone-700">{c.awards.subheadline}</p>
        </Reveal>
        <Stagger className="mt-10 grid gap-8 sm:mt-12 md:grid-cols-2 lg:grid-cols-4">
          {c.awards.items.map((a) => (
            <StaggerItem key={a.name} className="border-t-2 border-stone-900 pt-5">
      ...
      <section id="uslugi" className="mx-auto max-w-screen-xl border-b-2 border-stone-900 px-4 py-16 sm:px-6 sm:py-20 md:px-10">
        ...
        <RevealH2 className="mt-4 max-w-3xl font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-5xl sm:leading-[0.95] md:text-7xl">{c.services.headline}</RevealH2>
        <Stagger className="mt-10 grid gap-x-10 gap-y-10 sm:mt-12 sm:gap-y-12 md:grid-cols-2 lg:grid-cols-3">
          {c.services.items.map((s, i) => (
            <StaggerItem key={s.id} className="border-t-2 border-stone-900 pt-5">
      ...
      <section id="proces" className="bg-[#9B2C2C] text-[#F4EFE6]">
        <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 sm:py-20 md:px-10">
          ...
          <RevealH2 className="mt-4 max-w-4xl font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-5xl sm:leading-[0.95] md:text-7xl">{c.process.headline}</RevealH2>
          <Stagger stagger={0.1} className="mt-12 divide-y divide-[#F4EFE6]/30 border-y border-[#F4EFE6]/30 sm:mt-14">
            {c.process.steps.map((s) => (
              <StaggerItem key={s.number} className="grid gap-5 py-7 sm:gap-6 sm:py-8 md:grid-cols-12">
      ...
      <section id="faq" className="mx-auto max-w-screen-xl border-b-2 border-stone-900 px-4 py-16 sm:px-6 sm:py-20 md:px-10">
        <div className="grid gap-10 md:grid-cols-12 md:gap-12">
          <div className="md:col-span-4">
            ...
            <RevealH2 className="mt-4 font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-4xl sm:leading-[0.95] md:text-6xl">{c.faq.headline}</RevealH2>
            <Reveal delay={0.15}>
              <p className="mt-6 font-[Inter] text-base leading-relaxed text-stone-700">{c.faq.subheadline}</p>
            </Reveal>
          </div>
          <Stagger stagger={0.05} className="divide-y-2 divide-stone-900 border-y-2 border-stone-900 md:col-span-8">
            {c.faq.items.map((q) => (
        ...
        <div className="mx-auto max-w-screen-xl px-4 py-20 sm:px-6 sm:py-24 md:px-10 md:py-32">
          <div className="grid gap-10 md:grid-cols-12 md:gap-12">
            <div className="md:col-span-7">
              ...
              <RevealH2 className="mt-6 font-[DM_Serif_Display] text-3xl leading-[1.02] sm:text-5xl sm:leading-[0.95] md:text-7xl">{c.cta.primary.headline}</RevealH2>
              <Reveal delay={0.2}>
            ...
            <Stagger className="space-y-5 font-[Inter] sm:space-y-6 md:col-span-5">
              {c.contact.channels.map((ch) => (
                <StaggerItem key={ch.value}>
                  <a href={ch.href} className="block border-b border-[#F4EFE6]/30 pb-4 transition hover:border-[#F4EFE6]">
