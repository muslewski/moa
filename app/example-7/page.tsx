    ...
    <main className="min-h-screen bg-[#E6E9EF] text-slate-700 font-[Inter,system-ui,sans-serif]">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-12 md:px-8">
        <div className="grid gap-10 md:grid-cols-12 md:gap-14">
          <div className="md:col-span-7">
            ...
            <h1 className="mt-8 text-balance text-5xl font-semibold leading-[1.05] tracking-[-0.02em] text-slate-800 md:text-7xl">
              Unikatowe<br />
              stoiska targowe,<br />
              <span className="text-slate-500">które zostają w pamięci.</span>
            </h1>
            <p className="mt-8 max-w-xl text-lg leading-relaxed text-slate-600">{c.hero.subheadline}</p>
            <div className="mt-10 flex flex-wrap items-center gap-5">
              <Link href={c.hero.primaryCta.href} className="neu inline-flex items-center gap-3 bg-slate-800 px-7 py-4 text-sm font-semibold text-white transition active:shadow-none" style={{ background: "linear-gradient(145deg, #2c3e50, #1a252f)" }}>
                {c.hero.primaryCta.label}
                <ArrowRightIcon className="size-4" />
              </Link>
              <a href={c.hero.secondaryCta.href} className="neu px-6 py-4 text-sm font-semibold text-slate-700 transition active:shadow-none">
                {c.hero.secondaryCta.label}
              </a>
            </div>
          </div>
          <div className="md:col-span-5">
            <div className="neu overflow-hidden p-4">
        ...
        <div className="mt-14 grid grid-cols-2 gap-4 md:grid-cols-4">
          {c.hero.trustBar.badges.map((b) => (
            <div key={b.name} className="neu px-5 py-4">
      ...
      <section id="o-nas" className="mx-auto max-w-7xl px-4 py-24 md:px-8">
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-7">
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-tight text-slate-800 md:text-5xl">{c.about.headline}</h2>
            <div className="mt-8 space-y-5 text-lg leading-relaxed text-slate-600">
              <p>{c.about.lead}</p>
              {c.about.paragraphs.map((p) => <p key={p}>{p}</p>)}
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4 self-start md:col-span-5">
            {c.about.stats.map((s) => (
              <div key={s.label} className="neu p-6">
                <p className="text-5xl font-bold leading-none tracking-tight text-slate-800">{s.value}</p>
      ...
      <section id="nagrody" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
        <div className="grid gap-8 md:grid-cols-12">
          <div className="md:col-span-5">
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-tight text-slate-800 md:text-5xl">{c.awards.headline}</h2>
          ...
          <div className="grid grid-cols-2 gap-4 md:col-span-7">
            {[moaPhotos[0], moaPhotos[3], moaPhotos[1], moaPhotos[4]].map((src, idx) => (
              <div key={idx} className="neu overflow-hidden p-3">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {c.awards.items.map((a) => (
            <div key={a.name} className="neu p-7">
              <div className="neu-inset grid size-16 place-items-center">
                <p className="text-xl font-bold text-slate-700">{a.year}</p>
              </div>
              <p className="mt-6 text-base font-semibold text-slate-800">{a.name}</p>
      ...
      <section id="uslugi" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
        <div className="max-w-2xl">
          ...
          <h2 className="mt-4 text-3xl font-semibold leading-tight tracking-tight text-slate-800 md:text-5xl">{c.services.headline}</h2>
          <p className="mt-6 text-lg text-slate-600">{c.services.subheadline}</p>
        </div>
        <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {c.services.items.map((s) => (
            <article key={s.id} className="neu p-7">
              <div className="neu-inset grid size-14 place-items-center">
                <ServiceIcon name={s.icon} className="size-6 text-slate-700" />
              </div>
              <h3 className="mt-6 text-xl font-semibold text-slate-800">{s.title}</h3>
      ...
      <section id="proces" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
        ...
        <h2 className="mt-4 max-w-3xl text-3xl font-semibold leading-tight tracking-tight text-slate-800 md:text-5xl">{c.process.headline}</h2>
        <div className="mt-12 grid gap-5 md:grid-cols-5">
          {c.process.steps.map((s) => (
            <div key={s.number} className="neu p-6">
        ...
        <div className="mt-10 neu overflow-hidden p-3">
          <Image src={moaPhotos[2]} alt="MOA realizacja" width={2400} height={1000} className="aspect-[21/9] w-full rounded-2xl object-cover" />
        </div>
      </section>
      ...
      <section id="faq" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-4">
            ...
            <h2 className="mt-4 text-3xl font-semibold leading-tight text-slate-800 md:text-4xl">{c.faq.headline}</h2>
            <p className="mt-6 text-base leading-relaxed text-slate-600">{c.faq.subheadline}</p>
          </div>
          <div className="space-y-3 md:col-span-8">
            {c.faq.items.map((q) => (
              <details key={q.question} className="neu group p-6">
                <summary className="flex cursor-pointer list-none items-start justify-between gap-6">
                  <h3 className="text-lg font-semibold text-slate-800">{q.question}</h3>
                  <span className="neu grid size-10 shrink-0 place-items-center text-slate-700 transition group-open:rotate-45">
      ...
      <section id="kontakt" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
        <div className="neu p-10 md:p-16">
          <div className="grid gap-10 md:grid-cols-12">
            <div className="md:col-span-7">
              <p className="text-xs uppercase tracking-[0.25em] text-slate-500">{c.cta.primary.eyebrow}</p>
              <h2 className="mt-6 text-balance text-4xl font-semibold leading-[1.05] tracking-tight text-slate-800 md:text-6xl">{c.cta.primary.headline}</h2>
              <p className="mt-6 max-w-xl text-lg text-slate-600">{c.cta.primary.subheadline}</p>
              <div className="mt-10 flex flex-wrap items-center gap-4">
                <Link href={c.cta.primary.primaryButton.href} className="neu inline-flex items-center gap-3 px-7 py-4 text-sm font-semibold text-slate-800">
                  {c.cta.primary.primaryButton.label}
                  <ArrowRightIcon className="size-4" />
                </Link>
                <a href={c.cta.primary.secondaryButton.href} className="neu-inset px-6 py-4 text-sm font-semibold text-slate-700">
            ...
            <div className="space-y-4 md:col-span-5">
              <a href="tel:+48501027234" className="neu flex items-center gap-4 p-5">
                <div className="neu-inset grid size-12 place-items-center"><PhoneIcon className="size-5 text-slate-700" /></div>
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Telefon</p>
                  <p className="text-xl font-semibold text-slate-800">+48 501 027 234</p>
                </div>
              </a>
              <a href="mailto:office@moa.pl" className="neu flex items-center gap-4 p-5">
                <div className="neu-inset grid size-12 place-items-center"><MailIcon className="size-5 text-slate-700" /></div>
                <div>
                  <p className="text-xs uppercase tracking-[0.18em] text-slate-500">E-mail</p>
                  <p className="text-xl font-semibold text-slate-800">office@moa.pl</p>
                </div>
              </a>
      ...
      <footer className="mx-auto max-w-7xl px-4 pb-10 md:px-8">
        <div className="neu p-10">
          <div className="grid gap-10 md:grid-cols-12">
            <div className="md:col-span-4">
              <p className="text-2xl font-semibold tracking-tight text-slate-800">MOA Masters of Arts</p>
          ...
          <div className="mt-10 flex flex-col gap-2 border-t border-slate-300/50 pt-6 text-xs text-slate-500 md:flex-row md:items-center md:justify-between">
