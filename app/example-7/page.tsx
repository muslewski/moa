...
export default function Example7() {
  ...
  return (
    <main className="min-h-screen bg-[#E6E9EF] text-slate-700 font-[Inter,system-ui,sans-serif]">
      {/* Top bar — sticky */}
      <header className="sticky top-0 z-50 bg-[#E6E9EF]/90 px-4 pb-3 pt-4 backdrop-blur md:px-8">
      ...
      <section className="mx-auto max-w-7xl px-4 pt-10 md:px-8 md:pt-12">
        ...
        <Stagger className="mt-12 grid grid-cols-2 gap-3 sm:mt-14 sm:gap-4 md:grid-cols-4">
          {c.hero.trustBar.badges.map((b) => (
            <StaggerItem key={b.name} className="neu px-4 py-4 sm:px-5">
      ...
      <section id="o-nas" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-24">
        <div className="grid gap-10 md:grid-cols-12">
          <div className="md:col-span-7">
            ...
            <RevealH2 className="mt-4 text-2xl font-semibold leading-tight tracking-tight text-slate-800 sm:text-3xl md:text-5xl">{c.about.headline}</RevealH2>
            <Reveal delay={0.15} className="mt-6 space-y-5 text-base leading-relaxed text-slate-600 sm:mt-8 sm:text-lg">
              <p>{c.about.lead}</p>
              {c.about.paragraphs.map((p) => <p key={p}>{p}</p>)}
            </Reveal>
          </div>
          <Stagger className="grid grid-cols-2 gap-3 self-start sm:gap-4 md:col-span-5">
            {c.about.stats.map((s) => (
              <StaggerItem key={s.label} className="neu p-5 sm:p-6">
      ...
      <section id="nagrody" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
        <div className="grid gap-8 md:grid-cols-12">
          <div className="md:col-span-5">
            ...
            <RevealH2 className="mt-4 text-2xl font-semibold leading-tight tracking-tight text-slate-800 sm:text-3xl md:text-5xl">{c.awards.headline}</RevealH2>
            <Reveal delay={0.15}>
              <p className="mt-6 text-base leading-relaxed text-slate-600">{c.awards.subheadline}</p>
            </Reveal>
          </div>
          <Stagger className="grid grid-cols-2 gap-3 sm:gap-4 md:col-span-7">
            {[moaPhotos[0], moaPhotos[3], moaPhotos[1], moaPhotos[4]].map((src, idx) => (
              <StaggerItem key={idx} className="neu overflow-hidden p-2 sm:p-3">
        ...
        <Stagger className="mt-10 grid gap-4 sm:mt-12 sm:gap-5 md:grid-cols-2 lg:grid-cols-4">
          {c.awards.items.map((a) => (
            <StaggerItem key={a.name} className="neu p-5 sm:p-7">
      ...
      <section id="uslugi" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
        <div className="max-w-2xl">
          ...
          <RevealH2 className="mt-4 text-2xl font-semibold leading-tight tracking-tight text-slate-800 sm:text-3xl md:text-5xl">{c.services.headline}</RevealH2>
          <Reveal delay={0.15}>
        ...
        <Stagger className="mt-10 grid gap-4 sm:mt-14 sm:gap-5 md:grid-cols-2 lg:grid-cols-3">
          {c.services.items.map((s) => (
            <StaggerItem key={s.id} className="neu p-5 sm:p-7">
      ...
      <section id="proces" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
        ...
        <RevealH2 className="mt-4 max-w-3xl text-2xl font-semibold leading-tight tracking-tight text-slate-800 sm:text-3xl md:text-5xl">{c.process.headline}</RevealH2>
        <Stagger className="mt-10 grid gap-4 sm:mt-12 sm:gap-5 md:grid-cols-5">
          {c.process.steps.map((s) => (
            <StaggerItem key={s.number} className="neu p-5 sm:p-6">
        ...
        <RevealMedia className="mt-10 neu overflow-hidden p-2 sm:p-3">
      ...
      <section id="faq" className="mx-auto max-w-7xl px-4 py-16 md:px-8 md:py-20">
        <div className="grid gap-8 md:grid-cols-12 md:gap-10">
          <div className="md:col-span-4">
            ...
            <RevealH2 className="mt-4 text-2xl font-semibold leading-tight text-slate-800 sm:text-3xl md:text-4xl">{c.faq.headline}</RevealH2>
            <Reveal delay={0.15}>
          ...
          <Stagger stagger={0.06} className="space-y-3 md:col-span-8">
            {c.faq.items.map((q) => (
              <StaggerItem key={q.question}>
                <details className="neu group p-5 sm:p-6">
        ...
        <div className="neu p-6 sm:p-10 md:p-16">
          <div className="grid gap-8 md:grid-cols-12 md:gap-10">
            <div className="md:col-span-7">
              ...
              <RevealH2 className="mt-6 text-balance text-3xl font-semibold leading-[1.05] tracking-tight text-slate-800 sm:text-4xl md:text-6xl">{c.cta.primary.headline}</RevealH2>
              <Reveal delay={0.2}>
